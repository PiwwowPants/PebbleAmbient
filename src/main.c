...
static const int PLANET_OFFSET = 14;
static const int PLANET_SPACING = 10;
...
static void update_proc(Layer * layer, GContext * ctx) {
  ...
  /*// watch circle
  graphics_context_set_fill_color(ctx, (GColor)GColorDarkGrayARGB8);
  ...
  graphics_context_set_stroke_color(ctx, (GColor)GColorDarkGrayARGB8);
  ...
  for (int i = 0; i < N_PLANETS; ++i) {
    ...
    graphics_fill_circle(ctx, planet_location, PLANET_SIZES[i]);
  }
  ...
  graphics_fill_circle(ctx, GPoint(72,84), 8);
  graphics_context_set_fill_color(ctx, (GColor)GColorIcterineARGB8);
